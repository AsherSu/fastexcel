 ...
 */
@Slf4j
public class MergeCellImageTagHandler extends AbstractXlsxTagHandler {
    ...
    /**
     * 处理其他可能的图片相关元素，包括cellImage内嵌图片
     */
    private void handleOtherPictureElements(XlsxReadContext xlsxReadContext, String name, Attributes attributes) {
        switch (name.toLowerCase()) {
            case "cellimage":
            case "cellpicture":
                handleCellImage(xlsxReadContext, attributes);
                break;
            case "imagedata":
                handleImageData(xlsxReadContext, attributes);
                break;
            default:
     ...
     * 处理cellImage内嵌图片
     */
    private void handleCellImage(XlsxReadContext xlsxReadContext, Attributes attributes) {
        // 获取图片的关系ID
        ...
        // 获取单元格位置信息
        String cellRef = attributes.getValue("ref");
        if (StringUtils.isEmpty(cellRef)) {
            cellRef = attributes.getValue("cell");
        }
        ...
        Integer col = null;
        if (!StringUtils.isEmpty(cellRef)) {
            int[] position = parseCellReference(cellRef);
            if (position != null) {
                row = position[0];
                col = position[1];
            }
        ...
        // 如果没有位置信息，尝试从属性中获取
        if (row == null) {
            row = extractRowIndex(attributes);
        }
        if (col == null) {
            col = extractColumnIndex(attributes);
        }

        if (!StringUtils.isEmpty(rId)) {
            createPictureFromRelationship(xlsxReadContext, rId, row, col);
        }
    }

    /**
     * 处理imageData元素（另一种内嵌图片格式）
     */
    private void handleImageData(XlsxReadContext xlsxReadContext, Attributes attributes) {
        String rId = attributes.getValue("r:id");
        if (StringUtils.isEmpty(rId)) {
            rId = attributes.getValue("id");
        }

        // imageData通常不直接包含位置信息，需要从上下文获取
        String title = attributes.getValue("o:title");
        String alt = attributes.getValue("alt");

        if (!StringUtils.isEmpty(rId)) {
            // 使用当前解析状态的位置信息，如果没有则使用默认值
            Integer row = parseState.row1 >= 0 ? parseState.row1 : 0;
            Integer col = parseState.col1 >= 0 ? parseState.col1 : 0;
            createPictureFromRelationship(xlsxReadContext, rId, row, col);
        }
    }
     ...
     * 通过关系ID创建图片对象
     */
    private void createPictureFromRelationship(XlsxReadContext xlsxReadContext, String rId, Integer row, Integer col) {
        ...
        Optional.ofNullable(packageRelationshipCollection.getRelationshipByID(rId))
                .ifPresent(relationship -> {
                    try {
                        ...
                        log.warn("通过关系ID创建图片时发生错误: rId={}, error={}", rId, e.getMessage());
                    }
                });
    }

    /**
     * 解析单元格引用（如A1, B2等）为行列索引
     */
    private int[] parseCellReference(String cellRef) {
        ...
        try {
            ...
            if (matcher.matches()) {
                ...
                int row = Integer.parseInt(rowStr) - 1; // 转换为0基索引

                return new int[]{row, col};
            }
        } catch (Exception e) {
            log.debug("解析单元格引用失败: {}", cellRef);
        }
